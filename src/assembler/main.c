...
static void write_ihex_line(FILE *out, uint8_t length, uint16_t address,
...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    } else if ((strcmp(argv[i], "-hex") == 0 || strcmp(argv[i], "-h") == 0) && i + 1 < argc) {
      hex_out = fopen(argv[++i], "w");
    } else if (argv[i][0] == '-') {
      fprintf(stderr, "Unknown or incomplete argument: %s\n", argv[i]);
      fprintf(stderr, "Usage: %s [input.s] [-o output.bin] [-hex|-h output.hex]\n", argv[0]);
      return 1;
